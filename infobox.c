...
#endif
{
	...
	if (hh) {
		...
		if (gs_stringw==0)  f1 = (double)a/(double)b; 
		else f1 = (double)x1*(double)a/((double)gs_stringw*b);
		if (gs_stringh==0)  f2 = (double)a/(double)b;
		else f2 = (double)y1*(double)a/((double)gs_stringh*b);
		w = IBWIDTH(hh);
		h = IBHEIGHT(hh);
		if (f2<f1) f1 = f2;
		a = (int)(f1*100.0);
