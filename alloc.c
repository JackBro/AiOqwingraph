...
/* 			Modified by AiO											  */
/* 			24.02.2016												  */
/*  	Added feature to Iterate the active nodes for Qwingraph32	  */
/*																	  */
/**********************************************************************/
...
*		-- Added by AiO
*		Node xy Title
*	return ptr to the node title string or 0
*/
#ifdef ANSI_C
char * node_xy_title(long x, long y)
#else
char * node_xy_title(x,y)
long	x,y;
#endif
{
	GNODE tmp;
	tmp = search_xy_node(x,y);
	if(tmp == 0) return 0;
	return NLABEL(tmp);
}
...
/*****************************************************************
*		-- Added by AiO
*   Node Iterator for use by search function in QT C++ code
*	Global Variable:
*		bool last_Itor_node_error;
*		GNODE Node_Itorator;
*	Functions:
*		bool init_node_iterator()	//set to first node
*       bool next_node()	// t= more nodes, f= no more nodes
*		str *get_current_node_label()	//return a str*
*			 for current node filtered for label
*		long get_current_node_x()	// return current node x value
*		long get_current_node_y()	// return current node y value
*		bool get_current_node_error()	//Allow test for error
******************************************************************/

GNODE Node_Itorator;
int last_Itor_node_error;
...
#ifdef ANSI_C
int init_node_iterator(void)
#else
int init_node_iterator()
#endif
{
	// test for valid GNODE head
	if(nodelist == 0)
	{
		last_Itor_node_error = 1;
		return 0;
	}		
	Node_Itorator = nodelist;
	last_Itor_node_error = 0;
	return 1;
}
...
#ifdef ANSI_C
int get_current_node_error(void)
#else
int get_current_node_error()
#endif
{
	if(last_Itor_node_error == 1)
	{
		last_Itor_node_error = 0;
		return 1;
	}
...
#endif
{
	if(NNEXT(Node_Itorator) == 0) return 0;
	Node_Itorator = NNEXT(Node_Itorator);
	return 1;
}
...
static char node_label_buf[1024];
#ifdef ANSI_C
char *get_current_node_label(void)
#else
...
#endif
{
	...
	char *pColon;
	//test string pointer
	if( NLABEL(Node_Itorator) == 0)
	{
		last_Itor_node_error = 1;
		return 0;
	}
	if( NLABEL(Node_Itorator) == 0) return 0;
	strncpy(node_label_buf, NLABEL(Node_Itorator), 1024-1);
	node_label_buf[1024-1] = 0;
	//test for color token
	if( node_label_buf[0] == 0x0c) pbuf = &node_label_buf[3];
	else pbuf = &node_label_buf[0];
	pColon = strchr(node_label_buf, ':');
	if(pColon != 0)
	{
		pColon = pColon - 3;
		*pColon = 0;
...
*/
#ifdef ANSI_C
long get_current_node_x(void)
#else
long get_current_node_x()
#endif
{
	if(Node_Itorator == 0)
	{
	...
	return NX(Node_Itorator)- NWIDTH(Node_Itorator);
}
...
*/
#ifdef ANSI_C
long get_current_node_y(void)
#else
long get_current_node_y()
#endif
{
	if(Node_Itorator == 0)
	{
	...
	return NY(Node_Itorator) - NHEIGHT(Node_Itorator);
}
