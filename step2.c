...
ide503 _PP((void));;*/ static double gs_ide578 _PP((GNODE node)); static double gs_ide554 _PP((GNODE node)); /*;PP((int i));
i));;*/ static double gs_ide579 _PP((GNODE node)); static double gs_ide555 _PP((GNODE node)); /*;));
...
int i;;*/ = (double)i*(double)(gs_ide547 +1) +(double)(( node )->nhorder) /*;
{;*/ ; else (( node )->bary) /*; e;;*/ = (double)i*(double)(gs_ide547 +1); /*; e;
;;*/ e = (( node )->succ) /*;;;*/ ; while (e) { gs_ide546 (((( (( e )->kante) /*;
>nhorder) /+;R/SC;yy+NT >=0) ;*/ )->end) /*;>nhorder) /+;R/SC;yy+NT >=0) ;
*/ ) /*;>nhorder) /+;R/SC;yy+NT >=0) ;*/ ,i); e = (( e )->next) /*;/SC;
yy+NT = (double)i*(double)(gs_ide547 ;*/ ; } e = (( node )->pred) /*; = (double)i*(double)(gs_ide547 +1);
...
#ifdef ANSI_C
static double gs_ide578 (GNODE node)
#else
static double gs_ide578 (node) GNODE node;
#endif
...
R/SC;yy+NT ==0) r;*/ ; } return ( ((double) Sum) / ((double) (256*(( node )->outdegree) /*;
node )->succ) /+;R/SC;yy+NT ;;*/ )) ); }
#ifdef ANSI_C
static double gs_ide554 (GNODE node)
#else
static double gs_ide554 (node) GNODE node;
#endif
...
R/SC;yy+NT ==0) re;*/ ; } return ( ((double) Sum) / ((double) (256*(( node )->indegree) /*;
node )->pred) /+;R/SC;yy+NT ;;*/ )) ); }
#ifdef ANSI_C
static double gs_ide579 (GNODE node)
#else
static double gs_ide579 (node) GNODE node;
#endif
{ int i, leftpart, rightpart; ADJEDGE w; assert((node)); ; switch ((( node )->outdegree) /*;
/N;D+ node;;*/ ) { case 0: return(0.0); case 1: return((double) (( ((( (( (( node )->succ) /*;
leftpart, rightpart;;*/ )->kante) /*;leftpart, rightpart;;*/ )->end) /*;
leftpart, rightpart;;*/ ) /*;leftpart, rightpart;;*/ )->position) /*;
leftpart, rightpart;;*/ - (double)(( (( (( node )->succ) /*;ADJ+D/+ w;
...
) (( ((( (( (( node )->succ) /+;R/SC;yy;*/ ; return ( ((double) i) / 2.0); /*;) /+;R/SC;yy+NT )->kante) /+;R/SC;yy+NT
...
return ( ((double) i) * 2.0);;*/ ; } quicksort_save_array(i); if (i % 2) return((double)(( gs_ide569 [i/2] )->position) /*;
...
69 [i++] = ((( (( w )->kante) /+;R/SC;y;*/ ; return ( ((double)  ((( gs_ide569 [i/2-1] )->position) /*;
};*/ * rightpart +  (( gs_ide569 [i/2] )->position) /*;rt_save_array(i);
;*/ * leftpart))  / ((double) (leftpart+rightpart)) ); }
#ifdef ANSI_C
static double gs_ide555 (GNODE node)
#else
static double gs_ide555 (node) GNODE node;
#endif
{ int i, leftpart, rightpart; ADJEDGE w; assert((node)); ; switch ((( node )->indegree) /*;
/N;D+ node;;*/ ) { case 0: return(0.0); case 1: return((double) (( ((( (( (( node )->pred) /*;
leftpart, rightpart;;*/ )->kante) /*;leftpart, rightpart;;*/ )->start) /*;
leftpart, rightpart;;*/ ) /*;leftpart, rightpart;;*/ )->position) /*;
leftpart, rightpart;;*/ + (double)(( (( (( node )->pred) /*;ADJ+D/+ w;
;*/ )->kante) /*;ADJ+D/+ w;;*/ )->anchor) /*;ADJ+D/+ w;;*/ /256.0); case 2:  /*;(node));
...
R/SC;yy;*/ ; return ( ((double) i) / 2.0); } i = 0; w = (( node )->pred) /*;
